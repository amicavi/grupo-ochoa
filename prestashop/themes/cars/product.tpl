...
<div itemscope itemtype="http://schema.org/Product" class="underline-diamond">
	<div class="primary_block row">
		...
		{/if}
		<h2 class="category-product-title">{$product->category|escape:'html':'UTF-8'}</h2>
		<!-- left infos-->
							...
							{foreach from=$images item=image name=thumbnails}
								...
								<li id="thumbnail_{$image.id_image}"{if $smarty.foreach.thumbnails.last} class="last"{/if}>
									<a{if $jqZoomEnabled && $have_image && !$content_only} href="javascript:void(0);" rel="{literal}{{/literal}gallery: 'gal1', smallimage: '{$link->getImageLink($product->link_rewrite, $imageIds, 'large_default')|escape:'html':'UTF-8'}',largeimage: '{$link->getImageLink($product->link_rewrite, $imageIds, 'thickbox_default')|escape:'html':'UTF-8'}'{literal}}{/literal}"{else} href="{$link->getImageLink($product->link_rewrite, $imageIds, 'thickbox_default')|escape:'html':'UTF-8'}"	data-fancybox-group="other-views" class="fancybox{if $image.id_image == $cover.id_image} shown{/if}"{/if} title="{$imageTitle}">
										<img class="img-responsive" id="thumb_{$image.id_image}" src="{$link->getImageLink($product->link_rewrite, $imageIds, 'home_default')|escape:'html':'UTF-8'}" alt="{$imageTitle}" title="{$imageTitle}"itemprop="image" />
									</a>
								</li>
							{/foreach}
						{/if}
		...
		<div class="pb-right-column col-xs-12 col-md-5">
			...
			{if $PS_STOCK_MANAGEMENT}
							...
							<div id="availability_statut"{if ($product->quantity <= 0 && !$product->available_later && $allow_oosp) || ($product->quantity > 0 && !$product->available_now) || !$product->available_for_order || $PS_CATALOG_MODE} style="display: none;"{/if}>
								<span id="availability_value"{if $product->quantity <= 0 && !$allow_oosp} class="warning_inline"{/if}>{if $product->quantity <= 0}{if $allow_oosp}{$product->available_later}{else}{l s='Out of stock'}{/if}{else}{$product->available_now}{/if}</span>
							</div>
							{hook h="displayProductDeliveryTime" product=$product}
							<div class="warning_inline" id="last_quantities"{if ($product->quantity > $last_qties || $product->quantity <= 0) || $allow_oosp || !$product->available_for_order || $PS_CATALOG_MODE} style="display: none"{/if} >{l s='Warning: Last items in stock!'}</div>
						{/if}
			...
			<div class="content_prices clearfix">
						{if $product->show_price && !isset($restricted_country_mode) && !$PS_CATALOG_MODE}
							<!-- prices -->
							<div class="price {if $product->new && $product->on_sale && $product->specificPrice}mini-tags{/if} {if $productPriceWithoutReduction > $productPrice}reducer_pr{/if}">
			...
			{if ($display_qties == 1 && !$PS_CATALOG_MODE && $PS_STOCK_MANAGEMENT && $product->available_for_order)}
				...
				<li id="pQuantityAvailable"{if $product->quantity <= 0} style="display: none;"{/if}>
					<label {if $product->quantity > 1} style="display: none;"{/if} id="quantityAvailableTxt">{l s='Item:'}</label>
					<label {if $product->quantity == 1} style="display: none;"{/if} id="quantityAvailableTxtMultiple">{l s='Items:'}</label>
			...
			<form id="buy_block"{if $PS_CATALOG_MODE && !isset($groups) && $product->quantity > 0} class="hidden"{/if} action="{$link->getPageLink('cart')|escape:'html':'UTF-8'}" method="post">
				...
				<div class="box-info-product">
					...
					<div class="wrap-functional-btn row">
					<div class="wrap_quan clearfix" {if (!$allow_oosp && $product->quantity <= 0) || !$product->available_for_order || $PS_CATALOG_MODE} style="display: none;"{/if}>
						<label class="hidden-md">{l s='QTY:'}</label>
						<!-- quantity wanted -->
						{if !$PS_CATALOG_MODE}
						<div id="quantity_wanted_p"{if (!$allow_oosp && $product->quantity <= 0) || !$product->available_for_order || $PS_CATALOG_MODE} style="display: none;"{/if}>
							...
							<input type="number" name="qty" id="quantity_wanted" class="text" value="{if isset($quantityBackup)}{$quantityBackup|intval}{else}{if $product->minimal_quantity > 1}{$product->minimal_quantity}{else}1{/if}{/if}" />
						...
						<div class="box-cart-bottom {if (!$allow_oosp && $product->quantity <= 0) || !$product->available_for_order || (isset($restricted_country_mode) && $restricted_country_mode) || $PS_CATALOG_MODE} unvisible {/if}">
	...
	{if !$content_only}
		   {if (isset($product) && $product->description) || (isset($features) && $features) || (isset($HOOK_PRODUCT_TAB) && $HOOK_PRODUCT_TAB) || (isset($attachments) && $attachments) || isset($product) && $product->customizable}
			...
			<div id="more_info_block" class="clear underline-diamond">
				<ul id="more_info_tabs" class="info-tabs idTabs idTabsShort nav nav-tabs clearfix">
					...
					<li><a href="#customizable">{l s='Product customization'}</a></li>
					{/if}					
		...
		{if isset($attachments) && $attachments}
			<!--Download -->
			<section id="idTab3" class="bullet" class="page-product-box">
