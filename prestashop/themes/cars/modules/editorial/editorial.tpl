...
<!-- Module Editorial -->
<div id="editorial_block_center" class="editorial_block container text-center">
	...
	{if $homepage_logo}<img class="img-responsive logo-editorial" src="{$link->getMediaLink($image_path)|escape:'html'}" alt="{$editorial->body_title|escape:'html':'UTF-8'|stripslashes}" {if $image_width}width="{$image_width}"{/if} {if $image_height}height="{$image_height}" {/if}/>{/if}
	...
	{if $editorial->body_title}<h1 class="text-center">{$editorial->body_title|stripslashes}</h1>{/if}
