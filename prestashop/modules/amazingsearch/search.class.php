...
class Search extends SearchCore {
	public static function find($id_lang, $expr, $page_number = 1, $page_size = 1, $order_by = 'position', $order_way = 'desc', $ajax = false,
		$use_cookie = true, Context $context = null, $id_category = null)
	{
		if (is_null($id_category))
			$id_category = (int)Tools::getValue('category_id');
			if (!$context)
				$context = Context::getContext();
			if (!$use_cookie) $context->__set('validation_fix', 'validation_fix');
			...
			$sql = 'SELECT p.*, product_shop.*, stock.out_of_stock, IFNULL(stock.quantity, 0) as quantity, 
			 ...
			 MAX(image_shop.`id_image`) id_image, il.`legend`, m.`name` manufacturer_name '.$score.', MAX(product_attribute_shop.`id_product_attribute`) id_product_attribute,
				...
				'.($id_category > 0 ? ' INNER JOIN `'._DB_PREFIX_.'category_product` cp ON (cp.`id_product` = p.`id_product`)' : '').'
				LEFT JOIN `'._DB_PREFIX_.'image_lang` il ON (i.`id_image` = il.`id_image` AND il.`id_lang` = '.(int)$id_lang.')
				WHERE p.`id_product` '.$product_pool.'
				'.($id_category > 0 ? ' AND cp.`id_category`=  '.$id_category : '').'
